...
export class MysetsService implements OnInit {
	...
	removeActiveSet() {
		this.refreshLibrary();
		this.removeSet(this.activeSet.id).subscribe(
			(response) => {
				this.setMySetList(response);
				this.activeSet = null;
			},
			error1 => {
	...
	removeSet(setId: string): Observable<LyricSet[]> {
		return this.http.delete<LyricSet[]>(this.url + 'lyricsets/' + setId
		);
	}
	...
	getSet(setid: string): LyricSet {
		const ret = this.mysetlist.find(set => set.id === setid);
		if (!ret) {
			this.refreshLibrary();
		}
		return ret;
