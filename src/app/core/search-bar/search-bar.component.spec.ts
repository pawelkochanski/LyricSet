...
describe('SearchBarComponent', () => {
	...
	test('onInputFocusOut should set isAutocomplete to false', () => {
		component.onInputFocusOut();
		expect(component.isAutocomplete).toBeFalsy();
	});

	test('onKeyDown should set is Autocomplete to false if key is ENTER', () => {
		component.onKeyDown({keyCode: 13} as KeyboardEvent);
		expect(component.isAutocomplete).toBeFalsy();
	});

	test('onInputFocusIn should set isAutocomplete to true', () => {
		component.onInputFocusIn();
		expect(component.isAutocomplete).toBeTruthy();
	});
